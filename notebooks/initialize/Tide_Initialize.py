...
def plot_timeseries_with_interactive_controls(tide_gauge, eta_df, scheveningen):
    ...
    def plot_timeseries(date_range, **kwargs):
        ...
        axes[2].plot(eta_df[start_date:end_date]/100, color='darkorange', 
        ...
        axes[3].plot(tide_gauge[start_date:end_date].index, tide_gauge[start_date:end_date], color='black', 
                     label='Observed sea level', linewidth=0.7)
        axes[3].plot(eta_df[start_date:end_date]/100, color='darkorange', 
                     label='Total tidal signal', linewidth=0.5)
...
def questions_2d():
    ...
    general_question = ["1. Set the time range to around 30 days and select only the main semi-diurnal components (M2 and S2). Which phenomenon can you detect when looking at the combined signal of these two? What is the period of this beating (in days)? ", 
                     "2. Now select the M2 and K1 components. Looking at the combined signal, which phenomenon can you detect this time?", "3. Can you explain this phenomenon from the beating period of M2 and K1? Compute this beating period (in days)."
    ]
...
def questions_3a2():
    ...
    # make and display the questions
    general_question4 = ["1. In a semi-diurnal environment, spring tide occurs for tidal constituents TC1 and TC2 every D1 days. Set the time range to around 30 days, which phenomenon can you detect when looking at the combined signal of these two constituents?", 
...
def plot_2timeseries_with_interactive_controls(comps, dates, tide):
    ...
    all_comp = comps = ['M2', 'S2', 'N2', 'K2', 'K1', 'O1', 'P1', 'Q1', 'M3', 'M4', 'M6', 'M8', 'S4', 'MN4', 'MS4']
    # Define a list of checkboxes for component selection and put them in one row
    checkboxes = [widgets.Checkbox(value=(comp in ['M2']), description=comp, layout=widgets.Layout(width='auto')) for comp in comps]
    checkbox_row = widgets.HBox(checkboxes, layout=widgets.Layout(display='flex', flex_flow='row wrap'))
    ...
    def plot_timeseries(date_range, **kwargs):
        ...
        # Plot selected components
        fig, axes = plt.subplots(nrows=3, ncols=2,  figsize=(12, 7), sharex=True, constrained_layout=False)
        
        for comp in selected_components:
            axes[0, 0].plot(tide[locs[0]][comp.lower()][start_date:end_date], label=comp, linewidth=0.5)
            axes[0, 1].plot(tide[locs[1]][comp.lower()][start_date:end_date], label=comp, linewidth=0.5)
        l = axes[0, 1].legend(fontsize='small', loc='upper right', bbox_to_anchor=(1.2, 1))
        for line in l.get_lines():
            line.set_linewidth(3)
        axes[0, 0].set_title(f'{locs[0]}', fontweight='bold')
        axes[0, 1].set_title(f'{locs[1]}', fontweight='bold')
        ...
        axes[1, 1].plot(sum_values[1].index, sum_values[1], color='darkblue', 
                         label='Sum of selected components', linewidth=0.5)
        l1 = axes[1, 1].legend(fontsize='small', loc='upper right', bbox_to_anchor=(1, 1.2))
        ...
        for i, loc in enumerate(locs):
            sum_allvalues[i] = sum(tide[loc][comp.lower()][start_date:end_date] for comp in all_comp)
        axes[2, 0].plot(sum_allvalues[0].index, sum_allvalues[0], color='black', 
                 label='Total tidal signal', linewidth=0.5)
        axes[2, 1].plot(sum_allvalues[1].index, sum_allvalues[1], color='black', 
                         label='Total tidal signal', linewidth=0.5)
        l2 = axes[2, 1].legend(fontsize='small', loc='upper right', bbox_to_anchor=(1, 1.2))
...
def questions_4b():
    ...
    def multiple_choice_question_body(Question, options, answer):
        ...
        VBox1 = widgets.VBox([question_widget] + checkboxes + [submit_button, output_widget])
